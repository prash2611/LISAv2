...
# Build and install ntttcp with single one *.patch file
function build_ntttcp_with_patch () {
    git clone https://github.com/Microsoft/ntttcp-for-linux.git
    pushd ntttcp-for-linux/
    rm *.patch
    web_regex='(https?|ftp|file)://[-A-Za-z0-9\+&@#/%?=~_|!:,.;]*[-A-Za-z0-9\+&@#/%=~_|]'
    if [[ ${1} =~ $web_regex ]]; then
        wget ${1}
    fi
    patch_files=($(ls *.patch))
    if [[ ${#patch_files[@]} -eq 1 && -s ${patch_files[0]} ]]; then
        git apply "${patch_files[0]}"
        LogMsg "Info: successfully applied patch '${patch_files[0]}'"
        pushd src/ && make && make install
        popd
    else
        LogMsg "Error: no patch file found, or empty patch file, or multi patch files found, only one *.patch file supported"
    fi
    popd
}
...
function main {
    ...
    if [[ -z ${seconds_of_runtime} || -z ${monitor_interval} || -z ${tolerance} || (-z ${ntttcp_patch_file} && -z ${ntttcp_version}) ]]; then
        LogMsg "Error: missing parameter 'seconds_of_runtime' or 'monitor_interval' or 'tolerance' or (['ntttcp_patch_file'] | ['ntttcp_version'] "
        exit 1
    ...
    if [[ -n "${ntttcp_patch_file}" ]]; then
        build_ntttcp_with_patch "${ntttcp_patch_file}"
    else
    ...
    if [[ "${private_ip}" = "${server_internal_ip}" ]]; then
        ...
        last_line=($(tail -1 "${log_dir}"/monitor_ntttcp.log | sed -un '$ s/.*Real-time\sthroughput:\s\([0-9.]*\)\(.*bps\)/\1 \2/ p'))
        unit_baseline=${last_line[1]}
        throughput_baseline=${last_line[0]}
        ...
        do
            ...
            if [[ (1 = $(echo "${now_state[0]} != 0" | bc)) && (-n "${now_state[1]}") ]]; then
                if [[ (1 = $(echo "${now_state[0]} > ${threshold_upper} ||  ${now_state[0]} < ${threshold_lower}" | bc)) || (${now_state[1]} != ${unit_baseline}) ]]; then
                    mv "${log_dir}"/date_back_interval.log "${log_dir}"/out_of_tolerance@$(date +"%Y_%m_%d_%H_%M_%S")
                fi
                echo "EndTime: ${end_time_readable}, Baseline: ${throughput_baseline} ${unit_baseline},  Tolerance: +/- ${tolerance}, Now throughput: ${now_state[0]} ${now_state[1]}" > "${log_dir}"/state.txt
                SetTestStateRunning
