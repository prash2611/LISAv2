...
function Main {
	...
	if (!$serverStartedSuccessfully) {
		Write-LogErr "Start NTTTCP Server Failed."
		...
		while ($clientNtttcpStatus -ne "TestRunning" -and $sw.Elapsed -lt $timeout) {
		...
		if ($clientNtttcpStatus -eq "TestRunning") {
			...
			Write-LogErr "Start NTTTCP Client Failed."
			$currentTestResult.TestResult = $global:ResultFail
		}
	}
